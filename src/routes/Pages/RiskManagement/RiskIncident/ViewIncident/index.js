...
import { Preview } from '../CreateIncident';
...
const ViewIncident = props => {
  const { incidentDetails, openDrawer, onCloseDrawer, ...rest } = props;
  const [anchor] = useState('right');
  const classes = useStyles();
  const getColor = () => {
    let color;
    color = incidentDetails.status === 'Approved' ? 'green' : 'orange';
    return color;
  };
  return (
      ...
      <div className={clsx(classes.root)}>
        <Box className={classes.contentArea}>
          <PerfectScrollbar className={classes.scrollbarRoot}>
            ...
            <Box>
              <Preview {...{ incidentDetails, isView: true }} />
            </Box>
          </PerfectScrollbar>
        </Box>
      </div>
...
export default withWidth()(ViewIncident);
