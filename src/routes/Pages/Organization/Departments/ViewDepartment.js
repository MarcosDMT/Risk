...
const ViewDepartment = props => {
  const { deptDetails, openDrawer, onCloseDrawer, ...rest } = props;
  const [anchor] = useState('right');
  const classes = useStyles();
  const getColor = () => {
    let color;
    color = deptDetails.status === 'Approved' ? 'green' : 'orange';
    return color;
  };
  return (
      ...
      <div className={clsx(classes.root)}>
        <Box className={classes.contentArea}>
          <PerfectScrollbar className={classes.scrollbarRoot}>
            ...
            <Box>
              <DepartmentPreview {...{ deptDetails, isView: true }} />
            </Box>
          </PerfectScrollbar>
        </Box>
      </div>
...
export default withWidth()(ViewDepartment);
