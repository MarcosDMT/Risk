...
import { Preview } from '../AddEditCompliance';
...
const ViewCompliance = props => {
  const { complianceDetails, openDrawer, onCloseDrawer, isLegalCompliance, ...rest } = props;
  const [anchor] = useState('right');
  const classes = useStyles();
  const getColor = () => {
    let color;
    color = complianceDetails.submissionStatus === 'Active' ? 'green' : 'orange';
    return color;
  };
  return (
      ...
      <div className={clsx(classes.root)}>
        <Box className={classes.contentArea}>
          <PerfectScrollbar className={classes.scrollbarRoot}>
              ...
              <Box className={classes.headerContent}>
                <Box display={'flex'} alignItems={'center'}>
                  ...
                  <Chip
                    size={'small'}
                    variant={'default'}
                    label={complianceDetails.submissionStatus}
            ...
            <Box>
              <Preview {...{ complianceDetails, isView: true, isLegalCompliance }} />
            </Box>
          </PerfectScrollbar>
        </Box>
      </div>
...
export default withWidth()(ViewCompliance);
