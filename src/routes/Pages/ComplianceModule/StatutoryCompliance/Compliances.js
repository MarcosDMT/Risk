...
function AlertDialog({ data, displayValue }) {
...
export const Compliances = props => {
  ...
  const dispatch = useDispatch();
  const { statutoryCompliances } = useSelector(({ compliance }) => compliance);
  console.log(statutoryCompliances);
  ...
  function complianceStatus({ displayValue, data }) {
    let status = getComplianceStatus(displayValue);
    return <AlertDialog data={data} displayValue={displayValue} />;
  }
  ...
  return (
    <>
      <DataGrid
        dataSource={statutoryCompliances}
        showRowLines={true}
        showBorders={true}
