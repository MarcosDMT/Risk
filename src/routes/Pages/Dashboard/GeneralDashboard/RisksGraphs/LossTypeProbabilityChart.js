...
import ProbabilityImpactKey from './ProbabilityImpactKey';
...
const LossTypeProbabilityChart = props => {
  const { data, currentFilter, setCurrentFilter, filters, isLoading } = props;
    ...
    <CmtAdvCard>
      ...
      <CmtAdvCardContent className={classes.cardContentRoot}>
        <GridContainer>
          ...
          <Grid item xs={12} md={6}>
            <ProbabilityImpactKey data={data} />
          </Grid>
        </GridContainer>
      </CmtAdvCardContent>
    </CmtAdvCard>
          ...
          data.map((item, index) => (
            <Grid item xs={12} md={6} key={index}>
              <Box
                ...
                {...rest}>
                <Box component="span" ml={3} height={10} width={10} bgcolor={item.residualrisk_color} borderRadius="50%" />
                <Box>
