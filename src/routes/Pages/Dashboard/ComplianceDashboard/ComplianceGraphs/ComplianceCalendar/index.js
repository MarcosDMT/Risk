...
import { calendarCompliance } from '../../../dummyData';
import {useHistory} from "react-router-dom";

const ComplianceCalendar = () => {
  const [complianceEvents, setComplianceEvents] = useState(calendarCompliance);
  ...
  const getHeader = () => {
    ...
    return (
      <Box display="flex" flexDirection="column">
        <Box component={'p'} color="common.white" mb={4}>
  ...
  const getEvents = () => {
    return complianceEvents.filter(item => isDatesSame(item.date, date));
  };
  ...
  return (
    <CmtCard className={classes.cardRoot}>
      ...
      <CmtCardContent>
        ...
        {/*  <Button color="primary" className={classes.btnRoot} onClick={e => onViewAllClick()}>*/}
        {/*    View All*/}
        {/*  </Button>*/}
        {/*</Box>*/}
      </CmtCardContent>
