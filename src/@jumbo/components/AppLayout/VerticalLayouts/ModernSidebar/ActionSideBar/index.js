    ...
    <div className={clsx(classes.root, 'actionSidebar')}>
      ...
      <Box display="flex" flexDirection={{ xs: 'row', lg: 'column' }} ml={{ xs: 'auto', lg: 'unset' }}>
        ...
        <IconButton className={classes.iconBtn} onClick={() => onIconClick('notifications')}>
          <Badge badgeContent={4} overlap={'rectangular'} classes={{ badge: classes.counterRoot }}>
            <NotificationsIcon />
          </Badge>
        </IconButton>
      ...
      <Box display="flex" flexDirection={{ xs: 'row', lg: 'column' }} mt={{ xs: 'unset', lg: 'auto' }}>
        ...
        <CmtDropdownMenu
          ...
          TriggerComponent={
            <CmtAvatar
              src={authUser !== null && authUser.image_url !== undefined ? authUser.image_url : ''}
              alt={authUser !== null ? authUser.firstName : ''}
            />
          }
          items={actionsList}
        />
