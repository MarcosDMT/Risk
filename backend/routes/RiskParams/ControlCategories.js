...
const DELETE_URL = API_BASE_URL + '/api/v1/category';
...
router.get('/', async (req, res) => {
  ...
  try {
    await needle('get', `${API_URL}/categorycontrol`, options)
      .then(response => {
...
router.post('/create', async (req, res) => {
  ...
  try {
    await needle('post', `${API_URL}/categorycontrol`, formData, options)
      .then(response => {
        res.status(response.statusCode).json(response.body);
      })
      .catch(error => res.json(error));
